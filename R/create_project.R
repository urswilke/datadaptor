...
#' @param proj_template_path The template folder path;
#'   the first word should be the project leader initials.
#' @param mapping_subpath The subpath of the mapping file within
#'   `proj_template_path`.
#' @param proj_folder_name The project folder name
#'   (defaults to the path name `proj_template_path` without parent directories).
#' @param proj_name The project folder name with the project leader initials
#'   removed.
...
#' }
create_mapping_project <- function(
  new_proj_path,
  ...
  if (use_rproj) {
    ...
    starter_template <- list(
      ...
      rproj = rlang::expr(list(
        template_filename = system.file(
          "project_templates/default_rproj.Rproj",
          package = "starter"
        ),
      ...
      gitignore = rlang::expr(list(
        template_filename = system.file(
          "project-templates/default_gitignore.txt",
    ...
    if (use_renv) {
      ...
      renv::snapshot(
        new_proj_path,
        packages = c("datenanpassr", "crosstabser"),
        prompt = FALSE,
        force = TRUE
      )
    ...
    cli::cli_alert_success(
      "In your new Rstudio project type `usethis::use_git()` to commit the files."
    )
  }
}
