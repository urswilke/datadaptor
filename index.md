...
\
`devtools``::`[`install_git`](https://devtools.r-lib.org/reference/install-deprecated.html)`(``"https://codeberg.org/urswilke/datadaptor"``)`

Otherwise, you can also install datadaptor from github or gitlab with:

\
`devtools``::`[`install_github`](https://devtools.r-lib.org/reference/install-deprecated.html)`(``"urswilke/datadaptor"``)`\
`devtools``::`[`install_gitlab`](https://devtools.r-lib.org/reference/install-deprecated.html)`(``"urswilke/datadaptor"``)`
...
\
`spss_file`` ``<-`` `[`system.file`](https://rdrr.io/r/base/system.file.html)`(``"extdata"``, ``"fruit_survey.sav"``, package ``=`` ``"datadaptor"``)`\
`df`` ``<-`` ``haven``::`[`read_sav`](https://haven.tidyverse.org/reference/read_spss.html)`(``spss_file``)`\
`df`\
`#> # A tibble: 100 × 12`\
...
\
`mapping_file`` ``<-`` `[`system.file`](https://rdrr.io/r/base/system.file.html)`(``"extdata"``, ``"mapping-fruits.xlsx"``, package ``=`` ``"datadaptor"``)`

and then open it with:

\
`utils``::`[`browseURL`](https://rdrr.io/r/utils/browseURL.html)`(``mapping_file``)`
...
manipulate your data, you can generate a mapping object with

\
`mapping`` ``<-`` `[`Mapping`](https://urswilke.codeberg.page/datadaptor/reference/Mapping.md)`$``new``(``df``, ``mapping_file``)`
...
\
`haven``::`[`write_sav`](https://haven.tidyverse.org/reference/read_spss.html)`(``df_mod``, ``"fruit_survey-modified.sav"``)`
