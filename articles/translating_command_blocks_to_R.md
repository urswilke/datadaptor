...
\
`mapping_file`` ``<-`` `[`system.file`](https://rdrr.io/r/base/system.file.html)`(``"extdata"``, ``"mapping.xlsx"``, package ``=`` ``"datadaptor"``)`\
`spss_file`` ``<-`` `[`system.file`](https://rdrr.io/r/base/system.file.html)`(``"extdata"``, ``"mtcars_labelled.sav"``, package ``=`` ``"datadaptor"``)`\
`m`` ``<-`` `[`Mapping`](https://urswilke.codeberg.page/datadaptor/reference/Mapping.md)`$``new``(``spss_file``, ``mapping_file``)`
...
\
[`names`](https://rdrr.io/r/base/names.html)`(``m``$``cmd``$``command_blocks``[[``3``]``]``)`\
