...
[`library`](https://rdrr.io/r/base/library.html)`(`[`haven`](https://haven.tidyverse.org)`)`
...
SPSS sav file
[mtcars_labelled.sav](https://urswilke.codeberg.page/datadaptor/inst/extdata/mtcars_labelled.sav).
These are generated by [this Rmd
file](https://urswilke.codeberg.page/datadaptor/devel/make_fake_data.md).
...
`path`` ``<-`` `[`system.file`](https://rdrr.io/r/base/system.file.html)`(``"extdata"``, ``"mtcars_labelled.sav"``, package ``=`` ``"datadaptor"``)`\
[`read_sav`](https://haven.tidyverse.org/reference/read_spss.html)`(``path``)`\
`#> ``# A tibble: 32 × 13`\
`#>       id model         mpg cyl      disp    hp  drat    wt  qsec vs      am     `\
`#>    ``<dbl>`` ``<chr>``       ``<dbl>`` ``<dbl+l>`` ``<dbl>`` ``<dbl>`` ``<dbl>`` ``<dbl>`` ``<dbl>`` ``<dbl+l>`` ``<dbl+l>`\
`#> `` 1``     1 Mazda RX4    21   6`` [6 c…``  160    110  3.9   2.62  16.5 0`` [V-s…`` 1`` [man…`\
...
can be found
[here](https://urswilke.codeberg.page/datadaptor/inst/extdata/mapping.xlsx)
