...
make_assigns_cmd_table <- function(df_assigns, df_cats) {
  df_assigns_overview <-
    ...
    dplyr::group_by(q_id, var_ziel, EFA1MCG2MDG3, code_cat_join) %>%
...
verba_file = datenanpassr:::mapp_extract_verbatim_file(map_file, "Verbatims")
sheet = "Verbatims"
...
make_verbatim_cmd_table(map_file, verba_file = mapp_extract_verbatim_file(map_file, sheet), sheet = "Verbatims")
